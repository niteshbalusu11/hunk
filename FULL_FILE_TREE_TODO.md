...
- [x] Decide syntax strategy for editing path (use `gpui-component` code editor highlighter for MVP; revisit custom pipeline later).
- [x] Decide language parser strategy for future features (start with built-in parser/highlighter registry; evaluate custom `tree-sitter` integration later).
- [x] Define file limits and fallback behavior (very large files, binary files, non-UTF8).
...
- [x] Add `RightPaneMode::FileEditor` and routing between diff, preview, and editor modes.
...
- [x] Keep GPUI task lifecycle safe: no dropped tasks, epoch guards for stale async results.

### 2. Loading, saving, and safety
- [x] Replace read-only preview load path with editable document load path while retaining async I/O.
- [x] Add explicit save flow (`Cmd/Ctrl+S`) with atomic write (`temp file -> fsync -> rename` where applicable).
...
- [x] Add status messaging for save success/failure/conflict in existing toolbar/status surfaces.

### 3. Core editing interactions (MVP)
- [x] Add text insertion/deletion actions (`Backspace`, `Delete`, `Enter`, `Tab`, paste).
- [x] Add navigation actions (arrows, word movement, home/end, page up/down).
- [x] Add selection actions (shift+arrows, select-all, copy/cut/paste from focused editor).
- [x] Add line number gutter + current line highlight + horizontal/vertical scrolling.
...
- [x] On successful save, trigger snapshot refresh and preserve current selection/editor state.
- [ ] Handle deleted/renamed files gracefully when file tree refreshes.
- [ ] Define behavior for editing tracked vs untracked files.
- [x] Ensure diff tree mode remains unchanged and isolated from editor mode behavior.
