...
- Review surface snapshots now also carry file-header line stats plus per-row comment-affordance visibility/counts for visible rows, so comment UI no longer needs controller-side visible-row rescans during paint.
...
- Review sparse overlay anchors for file-header controls now ride on the shared surface snapshot instead of a render-time rescan over visible rows, which moves another piece of viewport composition onto one snapshot model.
- Review workspace sessions now also build the sparse overlay plan for visible file-header controls and the active comment editor anchor directly into the surface snapshot from one surface-options input, while row-level comment affordances stay in the viewport row payload.
