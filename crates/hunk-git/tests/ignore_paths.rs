...
fn filter_non_ignored_repo_paths_honors_gitignore_rules() -> Result<()> {
    ...
    fs::write(tempdir.path().join("target/output.o"), "artifact\n")?;
    ...
    let included = filter_non_ignored_repo_paths(
        ...
        &[
            ...
            ("target/output.o".to_string(), false),
    ...
    assert!(!included.contains("target"));
    assert!(!included.contains("target/output.o"));
